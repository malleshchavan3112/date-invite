'use client';

import { useState } from 'react';
import PrimaryButton from '@/components/ui/PrimaryButton';
import ChoiceCard from '@/components/ui/ChoiceCard';
import QuestionnaireProgress from './QuestionnaireProgress';
import DecorativeBackground from '@/components/ui/DecorativeBackground';
import DateInviteCard from '@/components/ui/DateInviteCard';

interface FoodPreferenceStepProps {
  value: string;
  onContinue: (food: string) => void;
  onBack: () => void;
  isEditingFromReview?: boolean;
}

const FOOD_OPTIONS = [
  { id: 'no_preference', label: 'No Preference',  emoji: '🍽️', description: 'Happy with anything' },
  { id: 'vegetarian',    label: 'Vegetarian',      emoji: '🥗', description: 'Plant-based goodness' },
  { id: 'vegan',         label: 'Vegan',           emoji: '🌱', description: 'Fully plant-based' },
  { id: 'seafood',       label: 'Seafood',         emoji: '🐟', description: 'From the ocean' },
  { id: 'street_food',   label: 'Street Food',     emoji: '🌮', description: 'Casual & delicious' },
  { id: 'fine_dining',   label: 'Fine Dining',     emoji: '🥂', description: 'Elegant & special' },
  { id: 'no_food',       label: 'No Food',         emoji: '🚫', description: 'This isn\'t about food' },
] as const;

/**
 * P12 — Food Preference (Phase 8)
 * Picks dietary preference or food style for the date.
 */
export default function FoodPreferenceStep({
  value,
  onContinue,
  onBack,
  isEditingFromReview = false,
}: FoodPreferenceStepProps) {
  const [selected, setSelected] = useState<string>(value);

  const handleContinue = () => {
    if (selected) {
      onContinue(selected);
    }
  };

  return (
    <div className="screen relative overflow-hidden min-h-dvh flex flex-col justify-center items-center px-4 py-8">
      {/* ── Ambient Decorative Background ── */}
      <DecorativeBackground />

      <div className="w-full max-w-md sm:max-w-xl md:max-w-2xl lg:max-w-[736px] mx-auto relative z-10">
        <DateInviteCard size="questionnaire">
          {/* Header progress & back */}
          <QuestionnaireProgress
            currentStep={7}
            totalSteps={9}
            onBack={onBack}
            isEditingFromReview={isEditingFromReview}
          />

          {/* Question & Supporting Copy */}
          <div className="mb-6 text-left">
            <h1 className="font-serif text-2xl sm:text-3xl text-dark mb-2 leading-snug font-normal text-balance">
              Any{' '}
              <span className="font-serif italic text-primary">food preferences</span>{' '}
              we should know?&nbsp;🍽️
            </h1>
            <p className="font-sans text-sm sm:text-base text-muted-foreground leading-relaxed text-balance">
              So we can pick somewhere you&apos;ll actually enjoy.
            </p>
          </div>

          {/* Choice Cards Grid */}
          <div
            className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 mb-6"
            role="radiogroup"
            aria-label="Food preference options"
          >
            {FOOD_OPTIONS.map((item) => (
              <ChoiceCard
                key={item.id}
                id={`food-${item.id}`}
                label={item.label}
                emoji={item.emoji}
                description={item.description}
                selected={selected === item.id}
                onSelect={() => setSelected(item.id)}
                className={item.id === 'no_food' ? 'sm:col-span-2' : ''}
              />
            ))}
          </div>

          {/* Continue CTA */}
          <PrimaryButton
            type="button"
            onClick={handleContinue}
            disabled={!selected}
            fullWidth
            id="food-continue-btn"
            className="py-3.5 text-base sm:text-lg shadow-button hover:shadow-button-hover"
          >
            {isEditingFromReview ? 'Save & Return to Review' : 'Continue'}
          </PrimaryButton>
        </DateInviteCard>
      </div>
    </div>
  );
}
